...
use NHK\QcBundle\Model\StockModel;
use NHK\QcBundle\Model\MonitoringModel;

class StockController extends Controller implements DomainObjectInterface
{
    public function indexAction(){
        $model = new MonitoringModel($this->getDoctrine()->getManager());
        $arrayReturn = array('items'=>$model->getAll());
        return $this->render('NHKQcBundle:Stock:stock.html.php',$arrayReturn);
    }

    public function addAction(){
        $model = new StockModel($this->getDoctrine()->getManager());
